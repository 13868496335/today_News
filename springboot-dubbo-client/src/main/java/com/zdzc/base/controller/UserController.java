...
import com.zdzc.base.utils.Response;
import com.zdzc.base.utils.SendMail;
...
public class UserController {
    ...
    public Response register(String t_user_account, String t_user_password, String t_user_email) {
        //service内容;
        Map<String,String> map=userService.regUser(t_user_account,t_user_password,t_user_email);
        if (map.get("ok") != null) {

            JavaMailSenderImpl sender = new JavaMailSenderImpl();
            sender.setHost("127.0.0.1");
            new SendMail("1",t_user_email,sender,1);
...
//       map.put("users",users);
    ...
    public ModelAndView userLogin(String t_user_account, String t_user_password, HttpSession session) {
        ...
        List<Map<String, Object>> list = userService.userLogin(t_user_account, t_user_password);
        Map t_user_id=userService.userLogin2(t_user_account,t_user_password);
        if (list.size() > 0){
...
//            session.setAttribute("t_user_url",t_user_url);
            ...
            //把菜单数据带首页-
            List nodes = userService.getUserNode(list.get(0).get("t_user_id").toString());
            //把菜单数据放到域
...
//            modelAndView.addObject("nodes",nodes);
    ...
    public ModelAndView UserHome(HttpSession session) {
        ...
        List<Map<String,Object>> users=userService.getUserHome(t_user_account);
    ...
    public ModelAndView deleteUser(String t_user_id) {
        ModelAndView mv = new ModelAndView();
        if (userService.deleteById(t_user_id)) {
            List users = userService.getAllUsers();
            mv.addObject("allusers", users);
            String success = "删除成功！";
    ...
    public ModelAndView updateUser(String t_user_id) {
        ModelAndView mv = new ModelAndView();
        List<Map<String, Object>> users = userService.queryUserById(t_user_id);
    ...
    public ModelAndView updateUser(String t_user_account, String t_user_password, String t_user_id) {
        ModelAndView mv = new ModelAndView();
        userService.editUser(t_user_account, t_user_password, t_user_id);
        List users = userService.getAllUsers();
        mv.addObject("allusers", users);
        mv.setViewName("/app/users.html");
        return mv;
    ...
    public ModelAndView saveUser(String t_user_account, String t_user_password) {
        ModelAndView mv = new ModelAndView();
        userService.insertUser2(t_user_account, t_user_password);
        List users = userService.getAllUsers();
        mv.addObject("allusers", users);
        mv.setViewName("/app/users.html");
        return mv;
    ...
    public void downloadAllClassmate(HttpServletResponse response) throws IOException {
        ...
        List<User> classmateList = userService.teacherinfor();
    ...
    public Map UpdateNewTX(String t_url,HttpSession session){
        Map map1=new HashMap();
        String t_user_account=session.getAttribute("t_user").toString();
        userService.userhomeimg(t_url,t_user_account);
        map1.put("error","success");
        return map1;
    }
}
